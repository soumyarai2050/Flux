...
const TreeWidget = (props) => {
    ...
    const onAutocompleteOptionChange = (e, value, xpath) => {
        let updatedData = cloneDeep(props.data);
        _.set(updatedData, xpath, value);
        props.onUpdate(updatedData);
    }
    ...
    let menu = (
        <>
            ...
            {isOpen ? (
                <Icon className={classes.icon} title='Collapse All' onClick={() => setIsOpen(false)}><UnfoldLess fontSize='small' /></Icon>
            ) : (
                <Icon className={classes.icon} title='Expand All' onClick={() => setIsOpen(true)}><UnfoldMore fontSize='small' /></Icon>
            )}
            {hide ? (
