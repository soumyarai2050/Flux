...
const TableNode = ({ data }) => {
    const { label, isSelected, hasChildren, isExpanded } = data;
...
const DataJoinGraph = ({ modelName, modelDataSource }) => {
    ...
    const calculateNodesAndEdges = useCallback((updatedContext = null) => {
        ...
        const allNodes = contextNodes.map((node, index) => {
            ...
            const isExpanded = !hasChildren || (hasChildren && expandedNodes.has(nodeName));
    ...
    const handleNodeSelection = useCallback(async (e, node) => {
        ...
        if (newSelection.length === 2 && mode === MODES.EDIT) {
            ...
            const sourceNode = contextNodes.find(n => get(n, graphAttributes.nodeNameField) === newSelection[0]);
            const targetNode = contextNodes.find(n => get(n, graphAttributes.nodeNameField) === newSelection[1]);

            if (sourceNode && targetNode) {
                // Get cached node data for columns
                const sourceName = get(sourceNode, graphAttributes.nodeNameField);
                const targetName = get(targetNode, graphAttributes.nodeNameField);
                const sourceNodeData = nodeDataCache[sourceName] || getCachedNodeData(sourceName);
                const targetNodeData = nodeDataCache[targetName] || getCachedNodeData(targetName);

                // Create node objects with columns
                const sourceNodeWithColumns = {
                    ...sourceNode,
                    columns: sourceNodeData?.columns || []
                };

                const targetNodeWithColumns = {
                    ...targetNode,
                    columns: targetNodeData?.columns || []
                };
    ...
    const handleModalSave = useCallback((saveData) => {
        ...
        // Create the new, complete edges array by combining the preserved edges
        // with the new/updated ones from the popup
        const newCompleteEdgesArray = [...otherEdges, ...updatedEdges];

        // Create updated context with modified nodes and edges
        const updatedContext = cloneDeep(activeContext);
        const contextNodes = get(updatedContext, graphAttributes.nodesPath) || [];

        // Update nodes with selected fields
        contextNodes.forEach(node => {
            const nodeName = get(node, graphAttributes.nodeNameField);
            if (nodeName === sourceName) {
                set(node, graphAttributes.nodeSelectedFieldsField, get(sourceNode, graphAttributes.nodeSelectedFieldsField));
            }
            if (nodeName === targetName) {
                set(node, graphAttributes.nodeSelectedFieldsField, get(targetNode, graphAttributes.nodeSelectedFieldsField));
            }
        });
    ...
    const handleEdgeClick = useCallback(async (event, edge) => {
        ...
        if (sourceNode && targetNode) {
            ...
            // Create node objects with columns for DataJoinPopup
            const sourceNodeWithColumns = {
                ...sourceNode,
                columns: sourceNodeData?.columns || [],
                [graphAttributes.nodeSelectedFieldsField]: get(sourceNode, graphAttributes.nodeSelectedFieldsField) || []
            };

            const targetNodeWithColumns = {
                ...targetNode,
                columns: targetNodeData?.columns || [],
                [graphAttributes.nodeSelectedFieldsField]: get(targetNode, graphAttributes.nodeSelectedFieldsField) || []
            };

            // Set modal nodes and open popup
            setModalNodes({
                source: sourceNodeWithColumns,
                target: targetNodeWithColumns
            });
                            ...
                            <Button variant="outlined"
                                ...
                                onClick={() => {
                                    if (selectedNodes.length === 2) {
                                        // Get the two selected nodes and their data
                                        const contextNodes = get(activeContext, graphAttributes.nodesPath) || [];
                                        const sourceNode = contextNodes.find(n => get(n, graphAttributes.nodeNameField) === selectedNodes[0]);
                                        const targetNode = contextNodes.find(n => get(n, graphAttributes.nodeNameField) === selectedNodes[1]);
