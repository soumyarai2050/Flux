...
# project imports
from Flux.CodeGenProjects.ui_proxy.app.ui_proxy_server_manager import UIProxyServerManager
...
if ui_proxy_projects_list is None or not isinstance(ui_proxy_projects_list, list) or not len(ui_proxy_projects_list):
    err_str = ("Couldn't find any project name in data/config.yaml file of ui_proxy_server project for "
               "loading ws route endpoints from generated config.yaml from projects")
...
async def handle_ui_get_all_ws(websocket: WebSocket) -> None:
    """ Get All websocket """
    if UIProxyServerManager.async_loop is None:
        UIProxyServerManager.async_loop = asyncio.get_running_loop()

    for ui_uri_to_server_uri_dict in ui_uri_to_server_uri_dict_list:
        if ui_uri_to_server_uri_dict.get("ui_uri") in str(websocket.url):
            server_ws_uri = ui_uri_to_server_uri_dict.get("server_uri")
            server_get_all_uri = ui_uri_to_server_uri_dict.get("server_http_uri")
            break
    else:
        err_str = (f"Can't find any ui_uri present in ui_uri_to_server_uri_dict_list that matched "
    ...
    is_new_ws = await UIProxyServerManager.register_n_connect(server_ws_uri, server_get_all_uri, websocket)
...
async def handle_ui_get_by_id_ws(websocket: WebSocket) -> None:
    ...
    for ui_uri_to_server_uri_dict in ui_uri_to_server_uri_dict_list:
        ui_uri_path = ui_uri_to_server_uri_dict.get("ui_uri").split("/")[1]
        if ui_uri_path in str(websocket.url):
            server_ws_uri = ui_uri_to_server_uri_dict.get("server_uri")
            server_get_by_id_http_uri = ui_uri_to_server_uri_dict.get("server_http_uri")
    ...
    is_new_ws = await UIProxyServerManager.register_n_connect(server_ws_uri, server_get_by_id_http_uri, websocket)
    await handle_ws(websocket, is_new_ws)
...
async def handle_ui_query_ws(websocket: WebSocket) -> None:
    ...
    for ui_uri_to_server_uri_dict in ui_uri_to_server_uri_dict_list:
        ui_uri_path = ui_uri_to_server_uri_dict.get("ui_uri")
        ws_url = str(websocket.url)
        if ui_uri_path in ws_url:
            server_ws_uri = ui_uri_to_server_uri_dict.get("server_uri")
            server_ws_uri += ws_url[ws_url.index("?"):] + "&need_initial_snapshot=false"    # Adding query params
            server_get_by_id_http_uri = ui_uri_to_server_uri_dict.get("server_http_uri") + ws_url[ws_url.index("?"):]
            break
    else:
        err_str = (f"Can't find any ui_uri present in ui_uri_to_server_uri_dict_list that matched "
                   f"path in uri {websocket.url}")
