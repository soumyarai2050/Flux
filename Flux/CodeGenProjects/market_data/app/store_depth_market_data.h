...
const std::string patch_uri = "/market_data_tech/patch-top_of_book/";
...
public:
    ...
    void create_data(std::string _id, double px_bid, double qty_bid, double px_ask, double qty_ask, std::string time, std::string symbol)
    {
        ...
        bodyStream << "{\"_id\": \"" << _id << "\",\"symbol\": \"" << symbol << "\",\"time\": \"" << time << "\"";
    ...
    void patch_data(std::string caller, std::string _id, double px_bid, double qty_bid, double px_ask, double qty_ask, std::string time, std::string symbol){
...
public:
    ...
    void handle_md_rt_update(MarketDataUpdateHolder &marketDataUpdateHolder_) {
        ...
        handle_md_update(position_symbol_key,
                         static_cast<const std::string>(marketDataUpdateHolder_.symbol),
                         static_cast<const std::string>(marketDataUpdateHolder_.side),
...
//            ptr_Pipeline_n_DB_id->top_of_book_id = top_of_book_id;
        ...
        for (auto&& depth_doc : market_depth_collection.find({})){
            std::ostringstream key_builder, update_key_builder;
            auto top_of_book_symbol = depth_doc[symbol_key].get_string().value;
            auto top_of_book_position = depth_doc[position_key].get_int32().value;
            auto top_of_book_side = depth_doc[side_key].get_string().value;
            key_builder << std::to_string(top_of_book_position) << "_" << top_of_book_side << "_" << top_of_book_symbol;
            auto top_of_book_position_symbol_side_key = key_builder.str();

            if (top_of_book_ui.find(top_of_book_position_symbol_side_key) == top_of_book_ui.end()) {
//                bsoncxx::document::view_or_value top_of_book_document = bsoncxx::builder::stream::document{}
...
//                top_of_book_collection.update_one(update_filter.view(), top_of_book_document.view());
                ...
                if (std::string (depth_doc[side_key].get_string().value) == "BID"){
                    ...
                    market_data_tech_ui_update.create_data(depth_doc[id_key].get_oid().value.to_string(),
                                                           depth_doc[px_key].get_double().value,
                                                           depth_doc[qty_key].get_double().value, 0, 0,
                                                           "2023-02-05T22:32:49.060Z",
                                                           std::string(depth_doc[symbol_key].get_string().value));
                ...
                auto top_of_book_id = depth_doc[id_key].get_oid().value.to_string();
                top_of_book_ui[top_of_book_position_symbol_side_key] = top_of_book_id;

            }else{
                market_data_tech_ui_update.patch_data(std::string (depth_doc[side_key].get_string().value),
                                                      top_of_book_ui.at(top_of_book_position_symbol_side_key), depth_doc[px_key].get_double().value,
                                                      depth_doc[qty_key].get_double().value,depth_doc[px_key].get_double().value,
                                                      depth_doc[qty_key].get_double().value, "2023-02-05T22:32:49.060Z",\
                                                          std::string(depth_doc[symbol_key].get_string().value));
            }
...
protected:
    ...
    std::unordered_map<std::string, std::string > top_of_book_ui;
    MongoDB &mongo_db;
    ...
    void replay(){
        for (auto &&history_document: market_depth_history_cursor) {
            ...
            marketDataHandler.handle_md_update(position_symbol_side_key,
                                               static_cast<const std::string>(history_document[symbol_key].get_string().value),
                                               static_cast<const std::string>(history_document[side_key].get_string().value),
                                               market_depth_document);
        }
    }
protected:
