...
from Flux.CodeGenProjects.market_data.generated.market_data_service_model_imports import BBOBaseModel


class StoreMarketDataClient(IbApiClient):
    ...
    def nextValidId(self, ticker_id: TickerId):
        ...
        """
        self.reqMarketDataType(StoreMarketDataClient.live_market_data)
    ...
    def _add_to_db(self, ticker_id: int, tick_type: str, px: float | None = None, qty: float | None = None):
        get_all_db_obj = self.market_data_service_web_client.get_all_bbo_client()
        ...
        for db_obj in get_all_db_obj:
            if db_obj.tick_type == tick_type:
                bbo = BBOBaseModel(id=db_obj.id,
                                   ...
                                   px=px, qty=qty)
                self.market_data_service_web_client.put_bbo_client(bbo)
            ...
            bbo = BBOBaseModel(req_id=ticker_id,
                               ...
                               tick_type=tick_type, date_time=DateTime.utcnow(),
                               px=px, qty=qty)
            self.market_data_service_web_client.create_bbo_client(bbo)
            logging.debug(f"Adding {bbo} in DB")
...
if __name__ == "__main__":
    def main():
        ...
        ib_client = StoreMarketDataClient(config_yaml)
        ib_client.setServerLogLevel(StoreMarketDataClient.log_lvl_detail)
