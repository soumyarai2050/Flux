...
from Flux.CodeGenProjects.AddressBook.ProjectGroup.contact_log_book.app.phone_book_base_tail_executor import (
...
from Flux.CodeGenProjects.AddressBook.ProjectGroup.phone_book.app.phone_book_service_helper import (
    email_book_service_http_client, get_reset_contact_log_book_cache_wrapper_pattern)
...
class PhoneBookTailExecutor(PhoneBookBaseTailExecutor):
    ...
    def __init__(self, log_detail, regex_file_dir_path: str,
        ...
        self.reset_contact_log_book_cache_pattern: str = get_reset_contact_log_book_cache_wrapper_pattern()
    ...
    def _handle_plan_alert_queue_err_handler(self, *args):
        try:
            ...
            contact_log_book_service_http_client.contact_alert_fail_logger_query_client(err_str_)
    ...
    def _handle_plan_alert_query_call_from_alert_queue_handler(self, plan_alerts: List[PlanAlertBaseModel]):
        ...
        contact_log_book_service_http_client.handle_plan_alerts_from_tail_executor_query_client(plan_alert_data_list)
        return plan_alerts
        ...
        try:
            ...
            if plan_id is None or self.plan_alert_cache_dict_by_plan_id_dict.get(plan_id) is None:
                ...
                update_plan_alert_cache(plan_id, self.plan_alert_cache_dict_by_plan_id_dict,
                                         contact_log_book_service_http_client.filtered_plan_alert_by_plan_id_query_client)
            ...
            if "Plan state changed from PlanState_ACTIVE to PlanState_PAUSED" in message:
                contact_log_book_service_http_client.plan_state_update_matcher_query_client(plan_id, message,
    ...
    def _send_plan_alerts(self, plan_id: int, severity_str: str, alert_brief: str,
        ...
        try:
            ...
            if self.plan_alert_cache_dict_by_plan_id_dict.get(plan_id) is None:
                ...
                contact_log_book_service_http_client.enable_disable_plan_alert_create_query_client(plan_id, True)
    ...
    def _handle_reset_contact_log_book_cache(self, matched_pattern):
        try:
            matched_pattern: str = matched_pattern.replace(self.reset_contact_log_book_cache_pattern, "").strip()
            ...
            contact_log_book_service_http_client.enable_disable_plan_alert_create_query_client(plan_id, False)

        except Exception as e:
            err_msg: str = (f"_handle_reset_contact_log_book_cache failed - cant clear cache"
    ...
    def handle_pair_plan_matched_log_message(self, log_prefix: str, log_message: str,
        ...
        # handling ResetContactLogBookCache
        if match := (
                re.compile(fr"{self.reset_contact_log_book_cache_pattern}(.*?){self.reset_contact_log_book_cache_pattern}"
                           ).search(log_message)):
            logging.info("Found ResetContactLogBookCache pattern")
            self._handle_reset_contact_log_book_cache(match[0])
            return
