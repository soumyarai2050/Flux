...
class ServiceState(BaseModel):
    ...
    def record_error(self, e: Exception) -> int:
        """
        returns time in seconds since first error if error is repeated, mobile_book otherwise
    ...
    def handle_exception(self, e: Exception, inspect_trace=None):
        ...
        if (last_error_interval_in_sec := self.record_error(e)) == mobile_book:
            # raise alert
            pass
        elif last_error_interval_in_sec > (6mobile_book * 5):
