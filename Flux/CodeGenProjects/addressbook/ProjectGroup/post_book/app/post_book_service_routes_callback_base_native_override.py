...
class PostBookServiceRoutesCallbackBaseNativeOverride(PostBookServiceRoutesCallback):
    ...
    def __init__(self):
        ...
        self.order_id_to_open_order_snapshot_cache_dict: Dict[str, OrderSnapshot] = {}
    ...
    async def get_last_n_sec_orders_by_events_query_pre(self, order_journal_class_type: Type[OrderJournal],
        ...
        return await PostBookServiceRoutesCallbackBaseNativeOverride.underlying_read_order_journal_http(
    ...
    async def get_open_order_count_query_pre(self, open_order_count_class_type: Type[OpenOrderCount], symbol: str):
        open_orders = await PostBookServiceRoutesCallbackBaseNativeOverride.underlying_read_order_snapshot_http(
    ...
    def load_existing_order_snapshot(self):
        run_coro = (
            PostBookServiceRoutesCallbackBaseNativeOverride.underlying_read_order_snapshot_http())
    ...
    def _update_open_order_snapshot_cache(self, order_snapshot: OrderSnapshot):
        order_id = order_snapshot.order_brief.order_id
        cached_open_order_snapshot = self.order_id_to_open_order_snapshot_cache_dict.get(order_id)
        if cached_open_order_snapshot is not None:
            if order_snapshot.order_status not in [OrderStatusType.OE_DOD, OrderStatusType.OE_FILLED]:
                self.order_id_to_open_order_snapshot_cache_dict[order_id] = order_snapshot
            else:
                # removing from cache if order is not open anymore
                del self.order_id_to_open_order_snapshot_cache_dict[order_id]
        else:
            if order_snapshot.order_status not in [OrderStatusType.OE_DOD, OrderStatusType.OE_FILLED]:
                self.order_id_to_open_order_snapshot_cache_dict[order_id] = order_snapshot
            # else not required: if order_id_to_open_order_snapshot_cache_dict doesn't contain this order_snapshot,
    ...
    def _load_existing_order_snapshot(self, order_snapshot_list: List[OrderSnapshot]):
        ...
        for order_snapshot in order_snapshot_list:
            ...
            # updating open_order_snapshots
            self._update_open_order_snapshot_cache(order_snapshot)
    ...
    async def create_or_update_order_snapshot(self, order_snapshot_list: List[OrderSnapshot]):
        async with OrderSnapshot.reentrant_lock:
            ...
            if create_order_snapshots:
                await PostBookServiceRoutesCallbackBaseNativeOverride.underlying_create_all_order_snapshot_http(
                    create_order_snapshots)
            if update_order_snapshots:
                await PostBookServiceRoutesCallbackBaseNativeOverride.underlying_update_all_order_snapshot_http(
    ...
    def add_order_journals(self, order_journal_list: List[OrderJournal]):
        run_coro = PostBookServiceRoutesCallbackBaseNativeOverride.underlying_create_all_order_journal_http(
            order_journal_list)
        future = asyncio.run_coroutine_threadsafe(run_coro, self.asyncio_loop)
    ...
    def _update_portfolio_status_n_check_portfolio_limits(self):
        while 1:
            ...
            while 1:
                try:
                    email_book_service_http_client.update_portfolio_status_by_order_or_fill_data_query_client(
    ...
    async def reload_cache_query_pre(self, reload_cache_class_type: Type[ReloadCache]):
        ...
        self.order_id_to_open_order_snapshot_cache_dict.clear()
        self.strat_id_to_strat_brief_cache_dict.clear()

        order_snapshot_list: List[OrderSnapshot] = \
            await PostBookServiceRoutesCallbackBaseNativeOverride.underlying_read_order_snapshot_http()
        self._load_existing_order_snapshot(order_snapshot_list)
