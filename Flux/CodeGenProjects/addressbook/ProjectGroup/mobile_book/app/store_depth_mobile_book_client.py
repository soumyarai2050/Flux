...
from Flux.CodeGenProjects.mobile_book.generated.mobile_book_service_model_imports import MarketDepthBaseModel, \
    RawMarketDepthHistoryBaseModel


class StoreDepthMobileBookClient(IbApiClient):
    ...
    def nextValidId(self, order_id: TickerId):
        ...
        """
        self.reqMobileBookType(StoreDepthMobileBookClient.live_mobile_book)
        contract: Contract
        for idx, contract in enumerate(self.contracts):
    ...
    def _update_mkt_depth(self, operation: int, market_depth_base_model: MarketDepthBaseModel):
        match operation:
            ...
            case 1:
                # operation 1: Update
                self.mobile_book_service_web_client.put_market_depth_client(market_depth_base_model)
            case 2:
                # operation 2: Delete
        ...
        if self.preserve_history:
            raw_market_depth_history_base_model = \
                RawMarketDepthHistoryBaseModel(symbol=self.contracts[ticker_id].symbol, time=current_time,
                                               ...
                                               px=price, qty=size)
            self.mobile_book_service_web_client.create_raw_market_depth_history_client(
                raw_market_depth_history_base_model)
...
if __name__ == "__main__":
    def main():
        ...
        ib_client = StoreDepthMobileBookClient()
        ib_client.setServerLogLevel(StoreDepthMobileBookClient.log_lvl_detail)

        host = StoreDepthMobileBookClient.config_yaml["host"]
        port = StoreDepthMobileBookClient.config_yaml["port"]
        client_id = StoreDepthMobileBookClient.config_yaml["client_id"]
