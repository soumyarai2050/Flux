...
class CppWebServerRoutesPlugin(BaseProtoPlugin):
    ...
    def generate_time_series_create(self, package_name: str, class_name: str, msg_name: str, mas_name_snake_cased: str,
        ...
        output_content += (f'\t\t\t\tm_{package_name}_publisher_.m_{mas_name_snake_cased}_codec_.'
                           f'get_data_by_id_from_collection({mas_name_snake_cased}, '
                           f'{mas_name_snake_cased}_db_id_);\n')
    ...
    def generate_create_method(self, package_name: str, class_name: str, msg_name: str, mas_name_snake_cased: str,
        ...
        output_content += (f'\t\t\t\t\tjson["_id"] = m_{package_name}_publisher_.'
                           f'm_{mas_name_snake_cased}_codec_.get_next_insert_id();\n')
        output_content += '\t\t\t\t}\n'
        output_content += f'\t\t\t\t{msg_name} {mas_name_snake_cased};\n'
        output_content += f'\t\t\t\t{class_name}JsonToObject::json_to_object(boost::json::serialize(json), {mas_name_snake_cased});\n'
        output_content += (f'\t\t\t\tauto inserted_id = m_{package_name}_publisher_.m_'
                           f'{mas_name_snake_cased}_codec_.insert({mas_name_snake_cased});\n')
    ...
    def generate_put_method(self, package_name: str, class_name: str, msg_name: str, mas_name_snake_cased: str,
        ...
        output_content += (f'\t\t\t\tauto inserted_id = m_{package_name}_publisher_.m_'
                           f'{mas_name_snake_cased}_codec_.patch({mas_name_snake_cased});\n')
    ...
    def generate_patch_method(self, package_name: str, class_name: str, msg_name: str, mas_name_snake_cased: str,
        ...
        output_content += '\t\t\t\t}\n\n'
        output_content += f'\t\t\t\t{msg_name} {mas_name_snake_cased};\n'
        output_content += (f'\t\t\t\tif (m_{package_name}_publisher_.m_{mas_name_snake_cased}_codec_.'
                           f'get_data_by_id_from_collection({mas_name_snake_cased}, patch_id)) {{\n')
        ...
        output_content += (f'\t\t\t\t\tauto inserted_id = m_{package_name}_publisher_.'
                           f'm_{mas_name_snake_cased}_codec_.patch(patch_id, json_obj);\n')
        output_content += f'\t\t\t\t\tif (!inserted_id) {{\n'
        output_content += f'\t\t\t\t\t\tres.result(boost::beast::http::status::bad_request);\n'
        output_content += f'\t\t\t\t\t\tboost::json::object json;\n'
        output_content += (f'\t\t\t\t\t\tjson["detail"] = "Id not Found: {msg_name} " + std::to_string('
    ...
    def generate_get_method(self, package_name: str, class_name: str, msg_name: str, mas_name_snake_cased: str,
        ...
        output_content += f'\t\t\tboost::json::object json;\n'
        output_content += (f'\t\t\tif (m_{package_name}_publisher_.m_{mas_name_snake_cased}_codec_.'
                           f'get_data_by_id_from_collection({mas_name_snake_cased}, stoi(match[1]))) {{\n')
    ...
    def generate_get_all_method(self, package_name: str, class_name: str, msg_name: str, mas_name_snake_cased: str,
        ...
        output_content += (f'\t\t\tm_{package_name}_publisher_.'
                           f'm_{mas_name_snake_cased}_codec_.get_data_from_collection_with_limit('
    ...
    def generate_delete_method(self, package_name: str, class_name: str, msg_name: str, mas_name_snake_cased: str,
                              msg: protogen.Message):
        output_content = ""
        output_content += (f'\tvoid delete_{mas_name_snake_cased}_by_id(boost::beast::http::request<boost::'
        ...
        output_content += (
            f'\t\t\tbool status = m_{package_name}_publisher_.m_{mas_name_snake_cased}_codec_'
            f'.delete_data_by_id_from_collection(stoi(match[1]));\n')
        output_content += f'\t\t\tboost::json::object json;\n'
    ...
    def generate_delete_all_method(self, package_name: str, class_name: str, msg_name: str, mas_name_snake_cased: str,
                              msg: protogen.Message):
        output_content = ""
        output_content += (f'\tvoid delete_all_{mas_name_snake_cased}(boost::beast::http::request<boost::beast::http'
                           f'::string_body>& req, boost::beast::http::response<boost::beast::http::'
                           f'string_body>& res, [[maybe_unused]] const std::smatch&) {{\n')
        output_content += f'\t\tif (req.method() == boost::beast::http::verb::delete_) {{\n'
        output_content += (f'\t\t\tbool status = m_{package_name}_publisher_.m_{mas_name_snake_cased}_codec_.'
                           f'delete_all_data_from_collection();\n')
        output_content += f'\t\t\tif (status) {{\n'
    ...
    def output_file_generate_handler(self, file: protogen.File):
        ...
        output_content += f'\tMarketDataPublisher& m_market_data_publisher_;\n'
        ...
        for msg in self.root_message_list:
            ...
            if self.is_option_enabled(msg, self.flux_msg_cpp_json_root):
                if self.is_option_enabled(msg, CppWebServerRoutesPlugin.flux_msg_json_root_time_series):
                    output_content += (f"\tint32_t {mas_name_snake_cased}_db_id_{{m_{package_name}_publisher_."
                                       f"m_{mas_name_snake_cased}_codec_.get_next_insert_id()}};\n\n")
        ...
        output_content += (f'\texplicit {class_name}WebServer(Config& config, MarketDataPublisher& market_data_publisher) : '
                           f'm_config_(config), m_timer_(m_ioc_), m_market_data_publisher_(market_data_publisher) {{\n\n')
        output_content += '\t\tif (start_connection()) {\n'
        output_content += f'\t\t\tm_{package_name}_thread_ = std::jthread(&{class_name}WebServer::run, this);\n'
        output_content += f'\t\t}}\n'
        output_content += '\t}\n\n'
        ...
        output_content += ('\tvoid serve_swagger_ui([[maybe_unused]] boost::beast::http::request<boost::beast::http::'
        ...
        output_content += ('\tvoid serve_swagger_json([[maybe_unused]] boost::beast::http::request<boost::beast::http::'
                           'string_body>& req, boost::beast::http::response<boost::beast::http::string_body>& res, '
                           '[[maybe_unused]] const std::smatch& match) {\n')
        ...
        output_content += '\tbool start_connection() {\n'
        output_content += '\t\ttry {\n'
