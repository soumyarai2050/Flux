...
class ModelComponentLoader(BaseJSLayoutPlugin):

    def __init__(self, base_dir_path: str):
        super().__init__(base_dir_path)

    def handle_lazy_load_components(self, file: protogen.File) -> str:
        output_str = ""
        for message in self.layout_msg_list:
            message_name = message.proto.name
            output_str += f"const {message_name} = lazy(() => import('../widgets/{message_name}'));\n"
        output_str += "\n"
        return output_str

    def handle_component_dictionary(self, file: protogen.File):
        output_str = ""
        for message in self.layout_msg_list:
            message_name = message.proto.name
            message_name_snake_cased = convert_camel_case_to_specific_case(message_name)
            output_str += f"'{message_name_snake_cased}': {message_name},\n"
        return output_str
    ...
    def output_file_generate_handler(self, file: protogen.File | List[protogen.File]):
        ...
        output_file_name = "modelComponentLoader.js"
        ...
        return {
            output_file_name: {
                "lazy_load_components": self.handle_lazy_load_components(file),
                "component_dictionary": self.handle_component_dictionary(file)
            }
        }


if __name__ == "__main__":
    main(ModelComponentLoader)
