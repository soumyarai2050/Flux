...
class JsxFileGenPlugin(BaseJSLayoutPlugin):
    ...
    def handle_import_output(self, message: protogen.Message, layout_type: str) -> str:
        if layout_type == JsxFileGenPlugin.repeated_root_type:
            output_str = "import React, { Fragment, useEffect, useState, memo, useRef, useCallback } from 'react';\n"
        else:
        ...
        if layout_type == JsxFileGenPlugin.repeated_root_type:
            ...
            output_str += ", setUserChanges, setDiscardedChanges, setOpenConfirmSavePopup, setChanges\n"
        else:
            output_str += "import {\n"
            output_str += f"    getAll{message_name}, get{message_name}, create{message_name}, update{message_name},\n"
        ...
        if layout_type == JsxFileGenPlugin.abbreviated_type:
            ...
            output_str += f"    set{dependent_message_name}Array, reset{dependent_message_name}, " \
                          ...
                          f"setUserChanges, setDiscardedChanges, setOpenConfirmSavePopup, setChanges\n"
            ...
            output_str += "import { createCollections, generateObjectFromSchema, addxpath, clearxpath, " \
                          ...
                          "createObjectFromXpathDict } from '../utils';\n"
            ...
            output_str += "import { \n"
            if layout_type == JsxFileGenPlugin.repeated_root_type:
                output_str += "    addxpath, getTableColumns, getTableRows, getCommonKeyCollections\n"
            elif layout_type == JsxFileGenPlugin.repeated_non_root_type:
                output_str += "    addxpath, getTableColumns, getTableRows, getCommonKeyCollections\n"
            else:
                output_str += "    createCollections, generateObjectFromSchema, addxpath, clearxpath, " \
                              "lowerFirstLetter, \n"
                if layout_type == JsxFileGenPlugin.root_type:
                    output_str += "    generateRowTrees, compareObjects, getObjectWithLeastId, hasxpath, " \
                                  ...
                                  "getXpathKeyValuePairFromObject, createObjectFromXpathDict\n"
                else:
                    output_str += "    generateRowTrees, compareObjects, getObjectWithLeastId, hasxpath, " \
                                  "getTableColumns, getTableRows, getCommonKeyCollections\n"
    ...
    def handle_non_abbreviated_return(self, message_name: str, message_name_camel_cased: str, layout_type: str) -> str:
        if layout_type == JsxFileGenPlugin.repeated_root_type:
            ...
            if layout_type == self.non_root_type:
                ...
                output_str += f"            updatedData[DB_ID] = selected{root_msg_name}Id;\n"
            ...
            if layout_type == self.root_type:
                ...
                output_str += "                updatedData = { ...updatedData, ...keyValueDict, [DB_ID]: " \
                              f"selected{root_msg_name}Id "+"};\n"
            output_str += "            } else {\n"
            if layout_type == self.root_type:
                output_str += "                updatedData = { ...updatedData, [xpath]: value };\n"
                output_str += "            }\n"
                output_str += "        }\n"
                output_str += "        setUserChanges(updatedData);\n"
            else:
                root_msg_name = self.root_message.proto.name
        ...
        if layout_type == JsxFileGenPlugin.root_type or layout_type == JsxFileGenPlugin.repeated_root_type or \
            ...
            output_str += "                src={changes}\n"
            output_str += "            />\n"
            output_str += "            <WebsocketUpdatePopup\n"
            output_str += "                title={title}\n"
            output_str += "                open={openPopup}\n"
    ...
    def __handle_const_on_layout(self, message_name: str, layout_type: str) -> str:
        ...
        match layout_type:
            ...
            case JsxFileGenPlugin.non_root_type:
                ...
                output_str += "    const { "+f"{message_name_camel_cased}Array, {message_name_camel_cased}, " \
                              f"modified{message_name}, selected{message_name}Id, loading, error, " \
                              f"mode, createMode, changes, userChanges, discardedChanges, openConfirmSavePopup " + \
                              "}"+f" = useSelector((state) => " \
            ...
            case JsxFileGenPlugin.abbreviated_type:
                ...
                output_str += "    const {"+f" {dependent_mesaage_camel_cased}Array, {dependent_mesaage_camel_cased}," \
                                            ...
                                            f"createMode, openConfirmSavePopup, changes, userChanges, discardedChanges " + \
                                            "}"+f" = useSelector(state => " \
                ...
                output_str += "    const [openPopup, setOpenPopup] = useState(false);\n\n"
                output_str += f"    const [{dependent_mesaage_camel_cased}SocketDict, " \
                              f"set{dependent_message}SocketDict] = useState("+"{});\n"
    ...
    def handle_abbreviated_return(self, message_name: str, message_name_camel_cased: str) -> str:
        ...
        output_str += "        dispatch(setChanges({ [xpath]: value }));\n"
        output_str += f"        if ({dependent_msg_name_camel_cased}[DB_ID]) "+"{\n"
    ...
    def handle_jsx_const(self, message: protogen.Message, layout_type: str) -> str:
        ...
        if layout_type != JsxFileGenPlugin.non_root_type:
            ...
            if layout_type == JsxFileGenPlugin.abbreviated_type:
                output_str += "    useEffect(() => {\n"
                abbreviated_dependent_msg_name_camel_cased = \
                    self.abbreviated_dependent_message_name[0].lower() + self.abbreviated_dependent_message_name[1:]
                output_str += f"        if (!createMode && selected{self.abbreviated_dependent_message_name}Id) "+"{\n"
                output_str += f"            let updatedData = addxpath(cloneDeep(" \
                              f"{abbreviated_dependent_msg_name_camel_cased}));\n"
        ...
        if layout_type == JsxFileGenPlugin.abbreviated_type:
            ...
            output_str += "            setLayout(currentSchema.widget_ui_data.layout);\n"
            output_str += "        }\n"
            output_str += "    }, [schema])\n\n"
            output_str += "    useEffect(() => {\n"
            output_str += f"        let updatedData = cloneDeep({message_name_camel_cased});\n"
            ...
            if layout_type == JsxFileGenPlugin.root_type:
                ...
                output_str += "    }\n\n"
                output_str += "    useEffect(() => {\n"
                output_str += f"        if (!createMode && selected{dependent_msg_name}Id) "+"{\n"
                output_str += f"            let updatedData = {dependent_msg_name_camel_cased}Array.filter(strat " \
                              f"=> strat[DB_ID] === selected{dependent_msg_name}Id)[0];\n"
                output_str += "            if (updatedData) {\n"
        ...
        if layout_type == JsxFileGenPlugin.abbreviated_type:
            ...
            output_str += "    }, [selected"+f"{message_name}Id])\n\n"
            output_str += "    useEffect(() => {\n"
            output_str += f"        if (selected{message_name}Id && _.get({message_name_camel_cased}, loadedKeyName)" \
                          f" && !_.isEqual({message_name_camel_cased}, previous{message_name})) "+"{\n"
            abbreviated_dependent_msg_snake_cased = \
                convert_camel_case_to_specific_case(self.abbreviated_dependent_message_name)
            abbreviated_dependent_msg_camel_cased = self.abbreviated_dependent_message_name[0].lower() + \
                                                        self.abbreviated_dependent_message_name[1:]
            output_str += f"            _.get({message_name_camel_cased}, loadedKeyName)." \
                          f"forEach(stratKey => "+"{\n"
            output_str += f"                let stratId = getIdFromAbbreviatedKey(abbreviated, stratKey);\n"
            output_str += f"                let socket = {abbreviated_dependent_msg_camel_cased}SocketDict[stratId];\n"
            output_str += "                if (!socket || (socket.readyState === WebSocket.CLOSING || " \
                          "socket.readyState === WebSocket.CLOSED)) {\n"
            output_str += "                    if (socket) socket.close();\n"
            output_str += "                    socket = new WebSocket(`${API_ROOT_URL.replace(" \
                          "'http', 'ws')}/get-"+f"{abbreviated_dependent_msg_snake_cased}"+"-ws/${stratId}`);\n"
            output_str += f"                    set{self.abbreviated_dependent_message_name}SocketDict(" \
                          "(prevState) => ({ ...prevState, [stratId]: socket }));\n"
            output_str += "                }\n"
            output_str += "                // close the websocket on re-render\n"
            output_str += "                return () => socket.close();\n"
            output_str += "            })\n"
            output_str += "        }\n"
            output_str += "    }, "+f"[selected{message_name}Id, {message_name_camel_cased}])\n\n"
            output_str += "    useEffect(() => {\n"
            ...
            output_str += f"                        dispatch(resetSelected{self.abbreviated_dependent_message_name}" \
                          f"Id());\n"
            output_str += f"                        dispatch(reset{self.abbreviated_dependent_message_name}());\n"
            output_str += f"                        dispatch(setModified{self.abbreviated_dependent_message_name}(" + \
                          "{}));\n"
            ...
            output_str += "    useEffect(() => {\n"
            output_str += f"        _.keys({abbreviated_dependent_msg_camel_cased}SocketDict)." \
                          f"forEach(socketId => "+"{\n"
            output_str += f"            socketId = parseInt(socketId);\n"
            output_str += f"            let socket = {abbreviated_dependent_msg_camel_cased}SocketDict[socketId];\n"
            output_str += "            if (socket) {\n"
            output_str += "                socket.onmessage = (event) => {\n"
            output_str += f"                    let updatedData = JSON.parse(event.data);\n"
            output_str += f"                    let updatedArray = {abbreviated_dependent_msg_camel_cased}Array." \
                          "filter(strat => {\n"
            output_str += f"                        return strat[DB_ID] !== socketId;\n"
            output_str += "                    });\n"
            output_str += f"                    updatedArray = [...updatedArray, updatedData];\n"
            output_str += f"                    dispatch(set{self.abbreviated_dependent_message_name}" \
                          f"Array(updatedArray));\n"
            output_str += f"                    if (updatedData[DB_ID] === selected" \
                          f"{self.abbreviated_dependent_message_name}Id) "+"{\n"
            output_str += f"                        let diff = compareObjects(updatedData, " \
                          f"modified{self.abbreviated_dependent_message_name}, " \
                          f"modified{self.abbreviated_dependent_message_name});\n"
            output_str += "                        let trees = generateRowTrees(cloneDeep(updatedData), " \
                          "dependentCollections);\n"
            output_str += "                        let modifiedTrees = generateRowTrees(" \
                          f"cloneDeep(modified{self.abbreviated_dependent_message_name}), dependentCollections);\n"
            output_str += "                        if (trees.length !== modifiedTrees.length) {\n"
            output_str += "                            if (mode === Modes.EDIT_MODE && _.keys(userChanges" \
                          ").length > 0) {\n"
            output_str += "                                setOpenPopup(true);\n"
            output_str += "                            } else {\n"
            output_str += "                                updatedData = addxpath(cloneDeep(updatedData));\n"
            output_str += f"                                dispatch(setModified" \
                          f"{self.abbreviated_dependent_message_name}(updatedData));\n"
            output_str += "                            }\n"
            output_str += "                        } else {\n"
            ...
            output_str += "                }\n\n"
            output_str += "                socket.onclose = (event) => {\n"
            output_str += f"                    let updatedData = cloneDeep(" \
                          f"{abbreviated_dependent_msg_camel_cased}SocketDict);\n"
            output_str += "                    delete updatedData[socketId];\n"
            output_str += f"                    set{self.abbreviated_dependent_message_name}SocketDict(updatedData);\n"
            output_str += "                }\n"
            output_str += "            }\n"
            output_str += "        })\n"
            output_str += "    }"+f", [{abbreviated_dependent_msg_camel_cased}Array, " \
                          f"{message_name_camel_cased}, {abbreviated_dependent_msg_camel_cased}SocketDict, " \
                          f"userChanges, {abbreviated_dependent_msg_camel_cased}, " \
                          f"modified{self.abbreviated_dependent_message_name}, mode])\n\n"
        ...
        if layout_type != JsxFileGenPlugin.abbreviated_type:
            if layout_type == JsxFileGenPlugin.root_type:
                ...
                output_str += "                let updatedData = JSON.parse(event.data);\n"
                output_str += "                let diff = compareObjects(updatedData, " \
                              f"modified{message_name}, modified{message_name});\n"
                output_str += "                if (_.keys(updatedData).length === 1) {\n"
                output_str += f"                    dispatch(resetSelected{message_name}Id());\n"
                output_str += f"                    dispatch(reset{message_name}());\n"
                output_str += "                } else {\n"
        ...
        if layout_type == JsxFileGenPlugin.abbreviated_type:
            ...
            output_str += "                dispatch(setChanges(userChanges));\n"
            output_str += "                dispatch(setOpenConfirmSavePopup(true));\n"
            output_str += "            } else {\n"
            output_str += "                onConfirmSave(null, false);\n"
            output_str += "            }\n"
            output_str += "        } else {\n"
            output_str += "            if (_.keys(changes).length > 0) {\n"
            output_str += "                dispatch(setOpenConfirmSavePopup(true));\n"
            output_str += "            }\n"
            output_str += "        }\n"
            output_str += "        if (openPopup) {\n"
            output_str += "            dispatch(setOpenConfirmSavePopup(true));\n"
            output_str += "        }\n"
            output_str += "    }\n"
            output_str += "    const onConfirmSave = (e) => {\n"
            output_str += "        let partialUpdate = false;\n"
            ...
            output_str += "            delete updatedData[DB_ID];\n"
            output_str += f"            dispatch(create{self.abbreviated_dependent_message_name}(" + \
                          "{ data: updatedData, abbreviated: abbreviated, loadedKeyName: loadedKeyName }));\n"
            output_str += "        } else {\n"
            output_str += f"            let updatedData = clearxpath(cloneDeep(modified" \
                          f"{self.abbreviated_dependent_message_name}));\n"
            abbreviated_dependent_msg_camel_cased = convert_to_camel_case(self.abbreviated_dependent_message_name)
            output_str += f"            if (!_.isEqual({abbreviated_dependent_msg_camel_cased}, updatedData)) "+"{\n"
            output_str += "                if (_.keys(changes).length === 1) {\n"
            output_str += "                    partialUpdate = true;\n"
            output_str += "                    let xpath = _.keys(changes)[0];\n"
            output_str += "                    let value = changes[xpath];\n"
            output_str += f"                    let updatedData = createObjectFromXpathDict(" \
                          f"{abbreviated_dependent_msg_camel_cased}, xpath, value);\n"
            ...
            output_str += f"                    dispatch(update{self.abbreviated_dependent_message_name}(" \
                          f"updatedData));\n"
            output_str += "                }\n"
            output_str += "            } else if (_.keys(changes).length === 1) {\n"
            output_str += "                partialUpdate = true;\n"
            output_str += "                let xpath = _.keys(changes)[0];\n"
            output_str += "                let value = changes[xpath];\n"
            output_str += f"                let updatedData = createObjectFromXpathDict(" \
            ...
            if layout_type == JsxFileGenPlugin.root_type:
                ...
                output_str += f"        dispatch(setModified{message_name}(updatedData));\n"
                output_str += f"        let changesDict = getXpathKeyValuePairFromObject(updatedData);\n"
                output_str += f"        setUserChanges(changesDict);\n"
                ...
                output_str += "        if (e) {\n"
                output_str += "            if (_.keys(userChanges).length > 0) {\n"
                output_str += "                setChanges(userChanges);\n"
                ...
                output_str += f"                if (_.keys(changes).length === 1) "+"{\n"
                output_str += "                    partialUpdate = true;\n"
                output_str += f"                    let xpath = _.keys(changes)[0];\n"
                output_str += f"                    let value = changes[xpath];\n"
                output_str += f"                    let updatedData = createObjectFromXpathDict(" \
                              f"{message_name_camel_cased}, xpath, value);\n"
        ...
        if layout_type == JsxFileGenPlugin.abbreviated_type:
            ...
            if layout_type == JsxFileGenPlugin.repeated_root_type:
                ...
                if layout_type != JsxFileGenPlugin.non_root_type:
                    ...
                    root_msg_name = self.root_message.proto.name
                    root_msg_camel_cased = convert_to_camel_case(root_msg_name)
                    output_str += "        dispatch(setChanges({ [xpath]: value }));\n"
                    output_str += f"        if ({root_msg_camel_cased}[DB_ID]) "+"{\n"
                    output_str += f"            onSave();\n"
                    output_str += "        }\n"
                    output_str += "    }\n\n"
        if layout_type == JsxFileGenPlugin.abbreviated_type:
