...
class JsSliceFileGenPlugin(BaseJSLayoutPlugin):
    ...
    def handle_slice_content(self, message: protogen.Message) -> str:
        ...
        if message_name == "UILayout":
            ...
            output_str += JsSliceFileGenPlugin.indentation_space*2 + "const { name, data } = action.payload;\n"
            output_str += (JsSliceFileGenPlugin.indentation_space*2 + "const layoutDataElement = state[storedObjKey]."
                           "widget_ui_data_elements.find((o) => o.i === name);\n")
            output_str += JsSliceFileGenPlugin.indentation_space*2 + "if (!data.bind_id_val) {\n"
            output_str += JsSliceFileGenPlugin.indentation_space*3 + "layoutDataElement.widget_ui_data.splice(0, 1, data);\n"
            output_str += JsSliceFileGenPlugin.indentation_space*2 + "} else {\n"
            output_str += (JsSliceFileGenPlugin.indentation_space*3 + "const idx = layoutDataElement.widget_ui_data.indexOf((o) => "
                           "o.bind_id_val === data.bind_id_val);\n")
            output_str += JsSliceFileGenPlugin.indentation_space*3 + "if (idx === -1) {\n"
            output_str += JsSliceFileGenPlugin.indentation_space*4 + "layoutDataElement.widget_ui_data.splice(idx, 1, data);\n"
            output_str += JsSliceFileGenPlugin.indentation_space*3 + "} else {\n"
            output_str += JsSliceFileGenPlugin.indentation_space*4 + "layoutDataElement.widget_ui_data.push(data);\n"
