...
class DataClassFastApiPlugin(FastapiCallbackFileHandler,
                             FastapiCallbackOverrideSetInstanceHandler,
    ...
    def load_root_and_non_root_messages_in_dicts(self, message_list: List[protogen.Message],
        ...
        for message in message_list:
            ...
            if ((is_json_root := self.is_option_enabled(message, DataClassFastApiPlugin.flux_msg_json_root)) or
                    self.is_option_enabled(message, DataClassFastApiPlugin.flux_msg_json_root_time_series)):
                if is_json_root:
                    json_root_msg_option_val_dict = \
                        self.get_complex_option_value_from_proto(message, DataClassFastApiPlugin.flux_msg_json_root)
                else:
                    json_root_msg_option_val_dict = \
                        self.get_complex_option_value_from_proto(message, DataClassFastApiPlugin.flux_msg_json_root_time_series)
                # taking first obj since json root is of non-repeated option
                if (is_reentrant_required := json_root_msg_option_val_dict.get(
                        DataClassFastApiPlugin.flux_json_root_set_reentrant_lock_field)) is not None:
                ...
                if DataClassFastApiPlugin.default_id_field_name in [field.proto.name for field in message.fields]:
            ...
            if self.is_option_enabled(message, DataClassFastApiPlugin.flux_msg_json_query):
    ...
    def _get_msg_id_field_type(self, message: protogen.Message) -> str:
        id_field_type: str = DataClassFastApiPlugin.default_id_type_var_name
        if message in self.custom_id_primary_key_messages:
            for field in message.fields:
                if field.proto.name == DataClassFastApiPlugin.default_id_field_name:
    ...
    def handle_init_db(self) -> str:
        ...
        for msg in self.root_message_list:
            ...
            if self.is_option_enabled(msg, DataClassFastApiPlugin.flux_msg_json_root_time_series):
            ...
            for field in msg.fields:
                if self.is_option_enabled(field, DataClassFastApiPlugin.flux_fld_index):
    ...
    def handle_fastapi_initialize_file_gen(self) -> str:
        output_str = "import os\n"
        output_str = "from dataclasses import dataclass\n"
        ...
        output_str += f"async def init_max_id_handler(data_class_type_model: Type[dataclass]):\n"
        output_str += f'    latest_obj = await data_class_type_model.collection_obj.find_one(sort=[("_id", -1)])\n'
        output_str += f'    if latest_obj is not None:\n'
        output_str += f'        max_val = latest_obj.get("_id")\n'
        output_str += f'    else:\n'
        output_str += f'        max_val = 0\n'
        output_str += f'    latest_obj = await data_class_type_model.collection_obj.find_one(sort=[("update_id", -1)])\n'
        output_str += f'    if latest_obj is not None:\n'
        output_str += f'        max_update_val = latest_obj.get("update_id")\n'
        output_str += f'    else:\n'
        output_str += f'        max_update_val = 0\n'
        output_str += f'    data_class_type_model.init_max_id(int(max_val), int(max_update_val))\n\n\n'
    ...
    def set_req_data_members(self, file: protogen.File):
        ...
        self.database_file_name = f"{self.proto_file_name}_dataclass_database"
        self.fastapi_file_name = f"{self.proto_file_name}_dataclass_fastapi"
        self.http_routes_file_name = f'{self.proto_file_name}_http_dataclass_routes'
        self.routes_callback_file_name = f"{self.proto_file_name}_routes_dataclass_callback"
        self.base_routes_file_name = f'{self.proto_file_name}_base_dataclass_routes'
        self.callback_override_set_instance_file_name = f"{self.proto_file_name}_dataclass_callback_override_set_instance"
        self.launch_file_name = self.proto_file_name + "_launch_dataclass_server"
        self.api_router_app_name = f"{self.proto_file_name}_API_router_dataclass"
        self.ws_routes_file_name = f'{self.proto_file_name}_ws_dataclass_routes'
    ...
    def output_file_generate_handler(self, file: protogen.File):
        ...
        output_dict: Dict[str, str] = {
            ...
            self.routes_callback_file_name + ".py": self.handle_dataclass_callback_class_file_gen(),
            #
            # Adding callback override set_instance file
            self.callback_override_set_instance_file_name + ".py":
                self.handle_dataclass_callback_override_set_instance_file_gen(),
            #
            ...
            self.http_routes_file_name + ".py": self.handle_http_dataclass_routes_file_gen(),
