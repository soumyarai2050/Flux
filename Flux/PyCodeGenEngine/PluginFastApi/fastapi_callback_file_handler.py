                ...
                for projection_option_val, query_name in projection_val_to_query_name_dict.items():
                    ...
                    for meta_field_name, meta_field_info in meta_data_field_name_to_field_tuple_dict.items():
                        if isinstance(meta_field_info, dict):
                            for nested_meta_field_name, nested_meta_type in meta_field_info.items():
                                meta_field_type_val, _ = nested_meta_type
                                query_param_with_type_str += f"{nested_meta_field_name}: {meta_field_type_val}, "
                        else:
                            meta_field_type_val, _ = meta_field_info
                            query_param_with_type_str += f"{meta_field_name}: {meta_field_type_val}, "
