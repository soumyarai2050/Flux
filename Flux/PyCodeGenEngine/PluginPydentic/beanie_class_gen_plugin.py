...
class BeanieClassGenPlugin(PydanticClassGenPlugin):
    ...
    def _handle_incremental_id_protected_field_override(self, message: protogen.Message) -> str:
        ...
        output_str += f"    id: int = Field(default_factory=(lambda: {message.proto.name}.next_id())," \
                      " description='Server generated unique Id', alias='_id')\n"
        return output_str
    ...
    def _check_id_int_field(self, message: protogen.Message) -> bool:
        ...
        for field in message.fields:
            if BeanieClassGenPlugin.default_id_field_name == field.proto.name:
                if 'int' != field.proto.name:
                    ...
                    logging.exception(err_str)
                    raise Exception(err_str)
        # else not required: if int id field doesn't exist in message then using default
    ...
    def _handle_cache_n_ws_connection_manager_data_members_override(self, message: protogen.Message, is_msg_root: bool):
        ...
        if is_msg_root:
            for field in message.fields:
                ...
                output_str += f"    _cache_obj_id_to_obj_dict: ClassVar[Dict[PydanticObjectId, " \
                              f"'{message.proto.name}']] = " + "{}\n"
