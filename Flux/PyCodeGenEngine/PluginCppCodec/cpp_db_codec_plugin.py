...
class CppDbHandlerPlugin(BaseProtoPlugin):
    ...
    def generate_public_members_handler(class_name: str, message_name: str, package_name: str,
        ...
        output_content += f"\t\t\tif ({message_name_snake_cased}_inserted_id == new_generated_id_out) {{"
        output_content += f"\n\t\t\t\t{message_name_snake_cased}_key_to_db_id" \
                          f"[{message_name_snake_cased}_key] = new_generated_id_out;" \
                          f"\n\t\t\t\treturn true;\n\t\t\t}} else {{\n"
        output_content += "\t\t\t\treturn false;\n\t\t\t}\n"
        output_content += "\n\t\t}"
    ...
    def generate_private_members_handler(class_name: str, package_name: str, message_name_snake_cased: str, file_name: str):
        output_content: str = ""
        output_content += f"\t\tstatic std::unordered_map <std::string, int32_t> {message_name_snake_cased}_key_to_db_id;\n\n"
        ...
        output_content += "\t\tstatic  std::mutex max_id_mutex;\n"
        output_content += "\t\tstatic int32_t cur_unused_max_id;\n\n"
    ...
    def generate_get_data_from_db(message_name: str, package_name: str, message_name_snake_cased: str, class_name: str):
        ...
        output_content += f"\n\t\tbool get_all_data_from_{message_name_snake_cased}_collection({package_name}::" \
                          f"{message_name}List &{message_name_snake_cased}_list_obj_out) {{\n"
        output_content += "\t\t\tbool status = false;\n"
    ...
    def generate_bulk_insert_and_update(message_name_snake_cased: str, message_name: str, package_name: str):
        ...
        output_content += f'\t\t\t\t}} else {{\n'
        output_content += (f'\t\t\t\t\t{message_name_snake_cased}_ids.emplace_back(std::move({message_name_snake_cased}'
    ...
    def generate_util_method_handler(message_name: str, message_name_snake_cased: str, class_name: str):
        ...
        output_content += "\t\t\tcur_unused_max_id++;\n"
        output_content += "\t\t\treturn cur_unused_max_id -1 ;\n"
        output_content += "\t\t}\n\n"

        output_content += f"\t\tstatic void init_next_{message_name_snake_cased}_insert_key() {{\n"
        output_content += "\t\t\tstd::lock_guard<std::mutex> lk(max_id_mutex);\n"
