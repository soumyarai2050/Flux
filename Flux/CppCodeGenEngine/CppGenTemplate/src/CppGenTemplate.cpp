...
class CppGenTemplate: public FluxCodeGenerator 
{
	public:
...
void CppGenTemplate::PrintField	(io::Printer &printer, const FieldDescriptor &field, 
										 std::map<std::string, std::string> & fieldAttributeMap) const
{
	...
	if (field.is_repeated()) 
	{
		if(isComplex)
		{
			...
			printer.Indent();
				...
				printer.Print(fieldAttributeMap,"qualified_cpp_type: `qualified_cpp_type`\n");
			printer.Outdent();
		}
		else
		{
	...
	else 
	{
		...
		else
		{
			...
			switch(field.type())
			{
				...
				{
					//Get Default
					//An int64 may or may-not be a date time. If its a datetime field, it should have FluxFldValDateTime option set
...
void CppGenTemplate::PrintMessage   (const Descriptor & messageDescriptor, io::Printer & printer) const
{
	...
	for (int i = 0; i < messageDescriptor.field_count(); ++i) 
	{
		...
		fieldAttributeMap["qualified_cpp_type"] = QualifiedCppTypeNameForField(field);
		
		PrintField(printer, field, fieldAttributeMap);
	}
}

void CppGenTemplate::PrintEnum   (const EnumDescriptor & enumDescriptor, io::Printer & printer) const
{
...
void CppGenTemplate::PrintMessages	(const FileDescriptor & fileDescriptor, io::Printer & printer) const
{
	//Steps:
	...
	for (int i = 0; i < fileDescriptor.message_type_count(); ++i) 
	{
		...
		printer.Print("qualified_cpp_name: `qualified_cpp_name`\n", "qualified_cpp_name", QualifiedCppClassOrEnumName(messageDescriptor));
		printer.Indent();
	...
	for (int i = 0; i < fileDescriptor.enum_type_count(); ++i) 
	{
		...
		printer.Print("qualified_cpp_name: `qualified_cpp_name`\n", "qualified_cpp_name", QualifiedCppClassOrEnumName(enumDescriptor));
		printer.Indent();
...
bool CppGenTemplate::Generate(const FileDescriptor* file,
				...
				std::string* error) const 
{
	...
	//Step 0.
	std::string strOutputFileName = GetFileNameFromFileDescriptor(file) + "_cpp.your_suffix";
	//Step 1.
