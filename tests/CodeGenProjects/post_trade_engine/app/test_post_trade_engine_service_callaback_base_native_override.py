...
# project imports
from Flux.CodeGenProjects.post_trade_engine.app.post_trade_engine_service_routes_callback_base_native_override import (
    PostTradeEngineServiceRoutesCallbackBaseNativeOverride, ContainerObject, OrderJournalBaseModel,
...
class MockPostTradeEngineServiceRoutesCallbackBaseNativeOverride(
        PostTradeEngineServiceRoutesCallbackBaseNativeOverride):
...
def test_queue_handling(single_strat_single_data, single_strat_multi_data,
    ...
    mock_post_trade_engine_override = MockPostTradeEngineServiceRoutesCallbackBaseNativeOverride()
    Thread(target=mock_post_trade_engine_override.portfolio_limit_check_queue_handler, daemon=True).start()

    # Checking single strat single data
    mock_post_trade_engine_override.expected_strat_id_list = [1]
    ...
    mock_post_trade_engine_override.expected_strat_id_to_container_obj_dict = {1: container_obj}
    # updating queue
    mock_post_trade_engine_override.portfolio_limit_check_queue.put(single_strat_single_data[0])
    mock_post_trade_engine_override.wait_semaphore.acquire()
    time.sleep(2)

    assert mock_post_trade_engine_override.is_strat_id_list, \
        (f"Mismatched: expected strat_id_list: {mock_post_trade_engine_override.expected_strat_id_list}, "
         f"found: {mock_post_trade_engine_override.found_strat_id_list}")
    assert mock_post_trade_engine_override.is_expected_strat_id_to_container_obj_dict, \
        (f"Mismatched: expected strat_id_to_container_obj_dict: "
         f"{mock_post_trade_engine_override.expected_strat_id_to_container_obj_dict}, "
         f"found: {mock_post_trade_engine_override.found_strat_id_to_container_obj_dict}")

    # Checking single strat multi data
    mock_post_trade_engine_override.expected_strat_id_list.clear()
    mock_post_trade_engine_override.expected_strat_id_to_container_obj_dict.clear()
    mock_post_trade_engine_override.is_strat_id_list = False
    mock_post_trade_engine_override.is_expected_strat_id_to_container_obj_dict = False
    mock_post_trade_engine_override.expected_strat_id_list = [1]
    order_journal_list = []
    order_snapshot_list = []
    strat_brief: StratBriefBaseModel | None = None
    ...
    # updating queue
    for payload in single_strat_multi_data:
        mock_post_trade_engine_override.portfolio_limit_check_queue.put(payload)
    mock_post_trade_engine_override.wait_semaphore.acquire()
    time.sleep(2)
    ...
    mock_post_trade_engine_override.is_strat_id_list = False
    mock_post_trade_engine_override.is_expected_strat_id_to_container_obj_dict = False
    mock_post_trade_engine_override.expected_strat_id_list = [1, 2, 3, 4, 5]
    for payload in multi_strat_single_data:
        ...
        container_obj = MockContainerObject(order_journals=[order_journal],
                                            order_snapshots=[order_snapshot],
                                            strat_brief=strat_brief)
        mock_post_trade_engine_override.expected_strat_id_to_container_obj_dict[strat_id] = container_obj
    # updating queue
    for payload in multi_strat_single_data:
        mock_post_trade_engine_override.portfolio_limit_check_queue.put(payload)
    mock_post_trade_engine_override.wait_semaphore.acquire()
    ...
    for payload in multi_strat_multi_data:
        ...
        container_obj = mock_post_trade_engine_override.expected_strat_id_to_container_obj_dict.get(strat_id)
