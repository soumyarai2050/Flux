...
#
#         config_file_path = STRAT_EXECUTOR / "data" / f"start_executor_{created_pair_strat.port}_config.yaml"
#
...
#         total_loop_count = 5
#         run_symbol_overview(buy_symbol, sell_symbol, symbol_overview_obj_list)
#         active_pair_strat = create_n_activate_strat(buy_symbol, sell_symbol, pair_strat_,
#                                                     expected_strat_limits_, expected_start_status_)
#
#         alert_id_list = []
#         broker_id_list = []
...
#         # creates and activates multiple pair_strats
#         for buy_symbol, sell_symbol in buy_sell_symbol_list:
#             activated_strat = create_n_activate_strat(buy_symbol, sell_symbol, pair_strat_, expected_strat_limits_,
#                                                       expected_start_status_)
...
#         for buy_symbol, sell_symbol in buy_sell_symbol_list:
#             run_symbol_overview(buy_symbol, sell_symbol, symbol_overview_obj_list)
#             activated_strat = create_n_activate_strat(buy_symbol, sell_symbol, pair_strat_,
#                                                       expected_strat_limits_, expected_start_status_)
#             # buy check
#             ongoing_strat_list = \
...
#
#
# def test_patch_all_(top_of_book_list_):
#     # bar_data = BarDataBaseModel(_id=1)
#     # bar_data.symbol_n_exch_id = SymbolNExchIdOptional()
#     # bar_data.symbol_n_exch_id.symbol = "s1"
#     # bar_data.symbol_n_exch_id.exch_id = "e2"
#     #
#     # json_data = jsonable_encoder([bar_data], by_alias=True, exclude_none=True)
#     # market_data_web_client.patch_all_bar_data_client(json_data)
#
#     tob = TopOfBookBaseModel(**top_of_book_list_[0])
#     tob.id = 2
#     tob.symbol = "s1"
#     # res = market_data_web_client.create_top_of_book_client(tob, return_obj_copy=False)
#
#     json_data = jsonable_encoder([tob], by_alias=True, exclude_none=True)
#     res = market_data_web_client.patch_all_top_of_book_client(json_data)
#     print(res, type(res))
#
#
# # @pytest.mark.asyncio
# def test_ws_client():
#     from Flux.CodeGenProjects.market_data.generated.FastApi.market_data_service_ws_client import MarketDataServiceWSClient
#     from FluxPythonUtils.scripts.ws_reader import WSReader
#
#     class SampleWSWebClient(MarketDataServiceWSClient):
#         def __init__(self):
#             super().__init__(host, parse_to_int(MARKET_DATA_BEANIE_PORT))
#             # self.check_projection = self.get_vwap_projection_from_bar_data_ws_client(notify=True,
#             #                                                                          symbol="s1", exch_id="e1")
#             self.check = self.query_filtered_notify_tob_update_ws_client(notify=True, symbols=["s1", "s2"])
#             WSReader.start()
#
#     sample_ws_client = SampleWSWebClient()
#
#     # await market_data_web_client.get_vwap_projection_from_bar_data_ws_query_client(check, "s1", "e1")
#     # await market_data_web_client.get_all_bar_data_client_ws(check)
#
#
...
def test_check():
    import traceback
    config_file_path = STRAT_EXECUTOR / "data" / f"strat_executor_8041_config.yaml"
    ...
    try:
        ...
        print(f"Some Error Occurred: exception: {e}, traceback: {''.join(traceback.format_exception(None, e, e.__traceback__))}")
        raise Exception(e)
    finally:
