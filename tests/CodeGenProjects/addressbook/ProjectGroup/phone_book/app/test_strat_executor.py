...
def test_no_strat_pause_on_order_fulfill_post_dod_if_config_is_false(
    ...
    try:
        ...
        pair_strat_obj = strat_manager_service_native_web_client.get_pair_strat_client(created_pair_strat.id)
...
def test_strat_pause_on_order_fulfill_post_dod_if_config_is_true(
    ...
    try:
        ...
        time.sleep(5)
        strat_alert = log_analyzer_web_client.get_strat_alert_client(created_pair_strat.id)
        for alert in strat_alert.alerts:
            if re.search(check_str, alert.alert_brief):
                break
        else:
